...
import routing.ProphetActiveCL;
import core.Message;
...
public class ProphetRouterTest extends AbstractRouterTest {
	...
	public void setUp() throws Exception {
		...
		ts.putSetting(ProphetActiveCL.PROPHET_NS + "." + 
				ProphetActiveCL.SECONDS_IN_UNIT_S , SECONDS_IN_TIME_UNIT+"");
		setRouterProto(new ProphetActiveCL(ts));
		super.setUp();
	}
	...
	public void testRouting() {
		...
		ProphetActiveCL r4 = (ProphetActiveCL)h4.getRouter();
		ProphetActiveCL r5 = (ProphetActiveCL)h5.getRouter();
		
		checkCreates(6);
		
		h4.connect(h5);
		assertEquals(ProphetActiveCL.P_INIT, r4.getPredFor(h5));
		assertEquals(ProphetActiveCL.P_INIT, r5.getPredFor(h4));
	...
	public void testAging() {
		...
		double newPred = ProphetActiveCL.P_INIT * Math.pow(ProphetActiveCL.GAMMA,2);
		
		assertEquals(newPred, r4.getPredFor(h5));
		assertEquals(newPred, r5.getPredFor(h4));
		
		clock.advance(SECONDS_IN_TIME_UNIT / 10);
		newPred = newPred *	Math.pow(ProphetActiveCL.GAMMA, 1.0/10);
