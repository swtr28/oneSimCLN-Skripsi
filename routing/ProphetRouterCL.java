 ...
 * Lindgren et al.
 */
public class ProphetRouterCL extends ActiveRouter implements CLDetectionEngine {
...
//    protected double msggenerationinterval = 600;
     ...
     */
    public ProphetRouterCL(Settings s) {
        super(s);
     ...
     */
    protected ProphetRouterCL(ProphetRouterCL r) {
        super(r);
    ...
    public void changedConnection(Connection con) {
        if (con.isUp()) {
            ...
            connectionDown(con);
            DTNHost otherHost = con.getOtherNode(getHost());
            ProphetRouterCL peerRouter = (ProphetRouterCL) otherHost.getRouter();
    ...
    private void updateTransitivePreds(DTNHost host) {
        ...
        Map<DTNHost, Double> othersPreds
                = ((ProphetRouterCL) otherRouter).getDeliveryPreds();
...
//			this.ratio = ratio;
    ...
    private Tuple<Message, Connection> tryOtherMessages() {
        ...
        for (Connection con : getConnections()) {
            ...
            ProphetRouterCL othRouter = (ProphetRouterCL) other.getRouter();
...
//                    Integer nrofCopies = (Integer) othRouter.getMessage(m.getId()).getProperty(MSG_COUNT_PROPERTY);
                    ...
                    if (cllist.size() != 0) {
//                        System.out.println("Cllist : " + cllist.size());
                        for (CLandTime cl : cllist) {
//                            System.out.println("Nilai CL bawah for : " + cl.CL);
                            if (cl.CL > 0 && cl.CL <= 0.5) {
//                                System.out.println("Nilai CL level 1 : " + cl.CL);
//                                System.out.println("TIDAK ADA CONGESTI");
                                messages.add(new Tuple<Message, Connection>(m, con));
                            } else if (cl.CL > 0.5 && cl.CL <= 1) { 
//                                 System.out.println("Nilai CL level 2 : " + cl.CL);
//                                System.out.println("CONGESTI SEDANG");
                                double ttlTersisa = (((m.getTtl())-(SimClock.getTime() - m.getCreationTime())) / (m.getTtl()));
//                                  System.out.println("TTL tersisa  : " + ttlTersisa);
                                if (ttlTersisa >= 0 && ttlTersisa <= 0.5) {
//                                    System.out.println("TTL tersisa dibawah 0. 3 : " + ttlTersisa);
//                                    System.out.println("MASUK TTL");
                                    messages.add(new Tuple<Message, Connection>(m, con));
                                }
                            } else if (cl.CL > 1) {
//                                System.out.println("Nilai CL level 1 : " + cl.CL);
//                                System.out.println("CONGESTI PARAH");
                                continue;
                            }
...
//                 messages.add(new Tuple<Message, Connection>(m, con));
    ...
    private class TupleComparator implements Comparator<Tuple<Message, Connection>> {
        ...
        public int compare(Tuple<Message, Connection> tuple1,
            ...
            double p1 = ((ProphetRouterCL) tuple1.getValue().
                    getOtherNode(getHost()).getRouter()).getPredFor(
                    tuple1.getKey().getTo());
            // -"- tuple2...
            double p2 = ((ProphetRouterCL) tuple2.getValue().
                    getOtherNode(getHost()).getRouter()).getPredFor(
                    tuple2.getKey().getTo());
...
//  
    ...
    public MessageRouter replicate() {
        ProphetRouterCL r = new ProphetRouterCL(this);
        return r;
    }
}
